...
function updateMenuDisplay(dateKey) {
  ...
  console.log(dayData);
  if (!dayData) {
    document.getElementById('dinner').textContent = 'No data available for this date.';
    return;
  }

  // Find Cafe 1919 data in the menu
  const cafe1919Menu = dayData.find(venue => 
    venue && venue.menuName && venue.menuName === "Café 1919 Menu Online Menu"
  );

  console.log(cafe1919Menu);
  if (!cafe1919Menu) {
    document.getElementById('dinner').textContent = 'No Cafe 1919 data available for this date.';
    return;
  }

  // Process menu data for dinner
  const processedData = {};

  // Process the Cafe 1919 menu
  if (!cafe1919Menu.menuWeeks || !Array.isArray(cafe1919Menu.menuWeeks)) {
    document.getElementById('dinner').textContent = 'No menu data available for Cafe 1919.';
    return;
  }
  
  cafe1919Menu.menuWeeks.forEach(week => {
  ...
  renderMealSections(processedData, 'dinner');
}
