...
function updateMenuDisplay(dateKey) {
  ...
  // Find Bruin Cafe data in the menu
  const bcafeMenu = dayData.find(venue => 
    venue && venue.menuName && venue.menuName === "Bruin Café Online Menu"
  );

  console.log(bcafeMenu);
  if (!bcafeMenu) {
    document.getElementById('lunch').textContent = 'No Bruin Cafe data available for this date.';
    return;
  }
  ...
  // Process the Bruin Cafe menu
  if (!bcafeMenu.menuWeeks || !Array.isArray(bcafeMenu.menuWeeks)) {
    document.getElementById('lunch').textContent = 'No menu data available for Bruin Cafe.';
    return;
  }
  
  bcafeMenu.menuWeeks.forEach(week => {
