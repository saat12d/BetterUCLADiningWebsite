...
function updateMenuDisplay(dateKey) {
  ...
  // Find Epic at Ackerman data in the menu
  const eaaMenu = dayData.find(venue => 
    venue && venue.menuName && venue.menuName === "Epic at Ackerman Online Menu"
  );

  console.log(eaaMenu);
  if (!eaaMenu) {
    document.getElementById('lunch').textContent = 'No Epic at Ackerman data available for this date.';
    return;
  }
  ...
  // Process the Epic at Ackerman menu
  if (!eaaMenu.menuWeeks || !Array.isArray(eaaMenu.menuWeeks)) {
    document.getElementById('lunch').textContent = 'No menu data available for Epic at Ackerman.';
    return;
  }
  
  eaaMenu.menuWeeks.forEach(week => {
