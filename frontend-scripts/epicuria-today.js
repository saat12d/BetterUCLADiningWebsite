...
function populateDateSelector(dates) {
  ...
  console.log('Populating date selector with dates:', dates); // Debug log
  ...
  console.log('Date selector populated with', selector.options.length, 'options'); // Debug log
}
...
function updateMenuDisplay(dateKey) {
  ...
  console.log('Menu data for selected date:', dayData); // Debug log
  ...
  const epicuriaDinner = dayData.find(venue => 
    venue && venue.menuName && venue.menuName === "Epicuria Dinner Service"
  );
  ...
  if (epicuriaLunch && epicuriaLunch.menuWeeks && Array.isArray(epicuriaLunch.menuWeeks)) {
    epicuriaLunch.menuWeeks.forEach(week => {
    ...
    console.warn('No Epicuria lunch menu found for date:', dateKey);
    document.getElementById('lunch-content').textContent = 'No lunch data available for this date.';
  }

  // Process the Epicuria Dinner menu
  if (epicuriaDinner && epicuriaDinner.menuWeeks && Array.isArray(epicuriaDinner.menuWeeks)) {
    epicuriaDinner.menuWeeks.forEach(week => {
      if (!week || !week.menuDays || !Array.isArray(week.menuDays)) return;
    ...
    console.warn('No Epicuria dinner menu found for date:', dateKey);
  ...
  try {
    ...
    console.error('Error rendering meal sections:', err);
  }
}
...
async function loadMenuData() {
  try {
    ...
    console.error('Error fetching Epicuria data:', err);
