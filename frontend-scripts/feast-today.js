...
function updateMenuDisplay(dateKey) {
  ...
  // Find Feast data in the menu
  const feastMenus = dayData.filter(venue => 
    venue && venue.storeName && venue.storeName === "Feast at Rieber"
  );

  if (!feastMenus || feastMenus.length === 0) {
    document.getElementById('dinner').textContent = 'No Feast data available for this date.';
    return;
  }
  ...
  // Process each Feast menu
  feastMenus.forEach(menu => {
    if (!menu || !menu.menuTypeName) return;
    
    const mealType = menu.menuTypeName;
    if (mealType === "Dinner") {
      // Process each menu week and day
      if (!menu.menuWeeks || !Array.isArray(menu.menuWeeks)) return;
